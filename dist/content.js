    ...
    async getValue(key) {
      return new Promise((resolve) => {
        extensionApi().storage.local.get([this.formatKey(key)], (result) => {
          const payload = result[this.formatKey(key)];
          if (!payload) {
            resolve(null);
            return;
          }
...
`;
  ...
  function parseTradeLocation(url) {
    ...
    const tradePattern = version2 === "2" ? /\/trade2\/([^/]+)\/([^/]+)(?:\/([^/]+))?/ : /\/trade\/([^/]+)\/([^/]+)(?:\/([^/]+))?/;
  ...
  function BookmarkTrade({ folderId, trade }) {
    ...
    const tradeUrl = buildTradeUrl({
      ...
      league: null
      // We don't store league in bookmark trades
    });
  ...
  async function initialize() {
    ...
    `;
      ...
      toggleRoot.render(
        /* @__PURE__ */ jsxRuntimeExports.jsx(reactExports.StrictMode, { children: /* @__PURE__ */ jsxRuntimeExports.jsx(CollapsedToggle, {}) })
      );
